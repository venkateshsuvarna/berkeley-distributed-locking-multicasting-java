#This python file is client 1 which can send messages to client 2 and 3 based on user choice
...
timestamp_client_1 = 0

def send_message(dummyparam1, dummyparam2):
	while True:
		print("Press 1 if you want to send a message to Client 2")
		...
		if(user_choice_int == 1):
			#Code to send a message to client 2
		
			try:
				machine = 2
				print ("Before s.connect")
				s=socket.socket()
				s.connect(("127.0.0.1",2000)) #DESKTOP-JH3J8A5
				print ("After s.connect")
				s.sendall(message.encode())
				time.sleep(5)
				s.sendall((str(machine)).encode())
				time.sleep(5)
				s.sendall((str(timestamp_client_1)).encode())
				time.sleep(5)
				s.close
...
def receive_message(message, machine, timestamp) :
	...
	timestamp_client_1 = (int)(timestamp_client_1)
	timestamp_client_1 = (int)(timestamp) + 1
	print("Timestamp of Machine 1 is : "+(str)(timestamp_client_1))
	return;
...
try:
	...
	timestamp_client_1 = 0
	print ("Initial Timestamp is " + str(timestamp_client_1))
